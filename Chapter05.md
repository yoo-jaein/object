...
- ```Controller``` : 컨트롤러 패턴
  ...
  > Solution: 유즈 케이스 컨트롤러가 유즈 케이스의 모든 시스템 이벤트를 다뤄야 한다. 혹은, 하나 이상의 유즈 케이스를 위해 사용되어야 한다.  
  
  예를 들어, 'Create User', 'Delete User'라는 유즈 케이스는 두 개의 분리된 유즈 케이스 컨트롤러 대신 하나의 UserController를 공통적으로 가진다.  
...
- ```Protected variations``` : 변경 보호 패턴
  > Problem: 의도하지 않은 변화를 최소화할 수 있도록 설계하는 방법은 무엇인가?   
...
0단계에서 만든 도메인 개념을 살펴보자. 도메인 상으로 Movie는 DiscountCondition의 목록을 속성으로 포함하고 있다. 두 개념은 이미 결합돼 있다. 따라서 Movie를 DiscountCondition과 협력하게 하면 설계 전체적으로 새로운 결합도를 추가하지 않고도 협력을 완성할 수 있다. 하지만 Screening이 DiscountCondition과 협력할 경우에는 Screening과 DiscountCondition 사이에 새로운 결합도가 추가된다. 따라서 전자가 더 나은 설계 대안인 것이다.
...
Reservation을 잘 알고 있거나, 긴밀하게 사용하거나, 초기화에 필요한 데이터를 가지고 있는 객체는 무엇인가? 바로 Screening이다. 2단계와 마찬가지로 이미 결합돼 있는 객체에게 생성 책임을 할당하는 것은 설계의 전체적인 결합도에 영향을 미치지 않는다. 결과적으로 이미 존재하는 관계를 이용하기 때문에 설계가 낮은 결합도를 유지할 수 있게 한다.

## 설계 4단계: 코드 작성하기
현재까지의 책임 분배는 설계를 시작하기 위한 대략적인 스케치에 불과하다. 실제 설계는 코드를 작성하는 동안 이뤄진다. 올바르게 설계하고 있는지 확인하는 방법은 코드를 작성하는 것 뿐이다. Screening부터 구현하자. Screening은 '예매하라' 메시지에 응답할 수 있어야 한다. 이 메시지를 처리할 메서드를 만들고, 책임을 수행하는 데 필요한 인스턴스 변수를 결정하자.
...
## 개선 1단계: 응집도를 낮추기 위해 클래스 분리하기
### 현재 문제점
DiscountCondition은 변경에 취약한 클래스이다. 다음과 같은 서로 다른 세 가지 이유로 클래스가 변경될 수 있다.  
...
현재 DiscountCondition은 위의 세 가지 조건에 모두 해당한다.  
...
예를 들어, 영화의 할인 정책을 실행 중에 변경할 수 있어야 한다는 요구사항이 추가됐다고 가정해 보자. 상속을 사용한 코드는 정책을 변경할 때 새로운 인스턴스를 생성한 후 필요한 정보를 복사해야 한다. 이 경우 새로운 할인 정책이 추가되었을 때 코드를 관리하기가 어렵다. 따라서 코드의 복잡성이 높아지더라도 상속 대신 합성을 사용하는 것이 더 좋은 방법이다. Movie의 상속 계층 안에 구현된 할인 정책 DiscountPolicy를 분리한 후 Movie에 합성시키면 유연한 설계가 완성된다.   
...
메서드를 다른 클래스로 이동시킬 때는 인자에 정의된 클래스 중 하나로 이동하는 경우가 일반적이다. DiscountCondition의 접근자 메서드를 사용하는 isDiscountable 메서드와 isSatisfiedBySequence 메서드를 DiscountCondition 내부로 옮기자. 이를 통해 DiscountCondition의 내부 구현을 캡슐화 할 수 있다. 또한, 할인 조건을 계산하는데 필요한 모든 로직이 한 곳에 모여있기 때문에 응집도 역시 높아졌다.

## 결론
1. 도메인 그리기 -> 정보 전문가 패턴 -> 높은 응집도 패턴과 낮은 결합도 패턴 -> 창조자 패턴 -> 코드 작성과 개선
2. 변경에 취약한 클래스란 코드를 수정해야 하는 이유를 하나 이상 가지는 클래스이다. 이 경우 클래스를 다형적으로 분리해야 한다.
3. 상속보다는 합성을 사용해야 한다.
4. 긴 메서드는 작고 응집도 높은 메서드로 분리하고, 각 메서드를 적절한 클래스로 이동시켜야 한다.
